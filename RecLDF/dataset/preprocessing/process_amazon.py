...
def generate_item_embedding(args, item_text_list, item2index, tokenizer, model, word_drop_ratio=-1):
    ...
    while start < len(order_texts):
        ...
        for sentence in sentences:
            ...
            last_part = parts[-1].strip() if parts[-1].strip() else '[PAD]'
            
            remaining_text = '. .'.join(parts[:-1]).strip()
            
            match = re.search(r'(.*?)(\d+.*)', remaining_text)
            if match:
                first_part = match.group(1).strip() if match.group(1).strip() else '[PAD]'
                middle_part = match.group(2).strip() if match.group(2).strip() else '[PAD]'
            else:
                first_part = remaining_text if remaining_text else '[PAD]'
                middle_part = '[PAD]'

            
            segments = [first_part, middle_part, last_part]
            segment_embeddings = []

            for segment in segments:
                if segment:
                    encoded_segment = tokenizer(segment, padding=True, max_length=512,
                                                truncation=True, return_tensors='pt').to(args.device)
                    outputs = model(**encoded_segment)
                    if args.emb_type == 'CLS':
                        cls_output = outputs.last_hidden_state[:, 0, ].detach().cpu()
                        segment_embeddings.append(cls_output)
                    elif args.emb_type == 'Mean':
                        masked_output = outputs.last_hidden_state * encoded_segment['attention_mask'].unsqueeze(-1)
                        mean_output = masked_output[:,1:,:].sum(dim=1) / \
                            encoded_segment['attention_mask'][:,1:].sum(dim=-1, keepdim=True)
                        mean_output = mean_output.detach().cpu()
                        segment_embeddings.append(mean_output)

            combined_embedding = torch.cat(segment_embeddings, dim=1).squeeze(0)
    ...
    all_embeddings.tofile(file)


def generate_item_embedding_1(args, item_text_list, item2index, tokenizer, model, word_drop_ratio=-1):
    print(f'Generate Text Embedding by {args.emb_type}: ')
    print(' Dataset: ', args.dataset)
...
def parse_args():
    ...
    parser.add_argument('--dataset', type=str, default='Pantry', help='Pantry / Scientific / Instruments / Arts / Office')
