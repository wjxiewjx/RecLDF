...
class Diffusion_Worker(nn.Module):
    ...
    def forward(self, x=None, cond_ts=None):
        ...
        noise = self.generate_ar_noise(x)
    ...
    def generate_ar_noise(self,x_start, lambda_value=0.5):
        device, dtype = x_start.device, x_start.dtype
        batch_size, feature_dim, seq_length = x_start.shape

        noise = torch.zeros_like(x_start, device=device, dtype=dtype)

        mid_index = seq_length // 2

        b = torch.randn(batch_size, feature_dim, device=device, dtype=dtype)
        noise[:, :, mid_index] = b

        lambda_value = torch.tensor(lambda_value, device=device, dtype=dtype)

        for i in range(seq_length):
            if i == mid_index:
                continue
            r_i = torch.randn(batch_size, feature_dim, device=device, dtype=dtype)
            noise[:, :, i] = torch.sqrt(lambda_value) * b + torch.sqrt(1 - lambda_value) * r_i
